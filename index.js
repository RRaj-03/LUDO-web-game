        ...
        array.forEach(element => {
            if (element.value == player.playerTurn) {
                ...
                if (canTokkenMove(nextIdNumber, currentIdNumber, element) == 2) {
                    ...
                    if (element.TokkenOnBoard == 1) {
                        let displayContainer = document.getElementById("diceCard");
                        displayContainer.removeChild(player.SelectedCardValue.card);
                        player.NoOfCards--
                    } else if (element.TokkenOnBoard == 2) {
                        let childs = document.querySelectorAll(`div.board.${element.value}`)
                        for (const child of childs) {
                            const parentId = child.parentElement.id;
                            const parent = document.getElementById(parentId)
                            const currentIdNumber = (parseInt(parentId.slice(3)))
                            let nextIdNumber = (parseInt(parentId.slice(3)) + parseInt(step))
                            if (canTokkenMove(nextIdNumber,currentIdNumber,element)) {
                                canPlayerMove=true
                            }
                        }
                        if (!canPlayerMove) {
...
function nextPlayerTurn() {
    ...
    array.forEach(element => {
        if (player.playerTurn == element.value) {
            let childs = document.querySelectorAll(`div.token.${element.value}`)
            for (const child of childs) {
                let parentid=child.parentElement.id;
                player.tokens.push(parentid)
                player.token.push(child)
            }
        }})
}
...
function generateNumber() {
  ...
  <div class="card-body" >
    ...
    if(player.noOf6==3){
        let displayContainer = document.getElementById("diceCard");
        let childs=displayContainer.childNodes
        while (childs[3]) {
            player.NoOfCards--
            player.noOf6--
            childs[3].remove()
        ...
        array.forEach(element => {
        if (player.playerTurn == element.value) {
            let childs1 = document.querySelectorAll(`div.token.${element.value}`)

            for (let index = 0; index < childs1.length; index++) {
                const child = childs1[index];
                child.remove()
                
                const parentid=player.tokens[index]
                const prevChild =player.token[index]
                const parent =document.getElementById(parentid)
                parent.appendChild(prevChild)
...
function StartGame() {
    ...
    let childs = document.querySelectorAll(`div.token.red`)
            for (const child of childs) {
                let parentid=child.parentElement.id;
                player.tokens.push(parentid)
                player.token.push(child)
